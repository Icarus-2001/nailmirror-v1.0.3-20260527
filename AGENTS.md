...
- **版本号**：小程序产品版 SemVer `1.1.x` / `1.2.x`（`app.globalData.version` 当前 **1.2.10**）；仓库目录名 `nailmirror-v1.6-*` 为里程碑代号，**不等于**小程序版本号。
...
- **登录与云函数**：`cloudfunctions/login` + `USE_CLOUD_LOGIN`（云失败降级 Mock）；登录页弹层 `chooseAvatar` + 昵称确认写入 `userStore`（**仅本地**，login 云函数不落库）；云函数须在开发者工具**手动上传部署**（推 Git 不更新云端）；`project.config.json` 的 `packOptions.ignore` 排除 `cloudfunctions/`、`tests/`、`scripts/` 等不进体验版包，仅上传小程序不会更新云端逻辑，云侧改动后须单独部署对应云函数；`app.json` 首屏登录页，已登录冷启动 `app.onLaunch` 跳首页（`skipLoginAutoRedirect`）；登录页须挂载 `privacy-popup` 并在选头像/昵称前完成隐私授权；`app.js` 须显式 `require` `tag-vocabulary`、`star-display` 等以免微信依赖图缺失报 `module is not defined`。
- **C 端体验**：首页试戴四步（甲型→选款→手照→预览），商详带 `styleId` 为三步（以 URL `styleId` 为准）；自定义参考款 `try-on-static` → `uploadStyleRef` → 云函数 `styleFileID`；试戴预览「换个款式试试」须 `wx.showModal` 确认后再换款合成；试戴历史真实款式名/标签与国内时间；合成等待 `utils/compose-waiting.js`（约 30s、`BRAND_LOGO`）；高清额度 `quota.service.js`（调试期 `ENABLE_FREE_HD_QUOTA: false`）；保存相册优先 `outputFileID`；「我的」商家经营入口未登录带 `from=merchant` 进登录页，成功后 `redirectTo` `pages-b/entry`，已登录直达 entry；`pages-b/stock-advice` 已于 1.2.4 从 `app.json` 移除，开发者工具旧缓存可能仍编译该页导致 ENOENT，磁盘保留兼容桩或清缓存；协作见 `docs/COLLABORATION_SOP.md` 等。
- **款式来源与上传**：平台 25 款 `styleSource: platform`，本地 `styles.real.js`+外链 CDN，非云存储；B 端商家 `uploadMerchantStyles` 入库云库 `styles` 并进款式库（徽章「来自商家」），云存储 `merchant/styles/`，`listMerchantStyles` 全员可见；C 端 `uploadStyleRef` 仅一次性试戴不入库；小红书热款 `styleSource: xhs-hot`、徽章「全网热款」（区别于平台特供/商家），`ops` `importXhsHotTop10`/`listXhsHotStyles`，热款榜仅 `{scrape_date} 全网热款 TOP10`（已移除「热门搜索词」mock）且款式库同步可见，封面 C 端优先 `cloud://` `image_file_id`（真机勿依赖 HTTPS 临时链；开发者工具 `urlCheck:false` 会掩盖域名问题），云侧封面修复后须重新部署 `ops` 并视情况重传体验版；商家/热款 `ensureMerchantStyles`/`ensureXhsHotStyles` 有约 10 分钟内存缓存，跨用户可见最坏延迟约 10 分钟。
