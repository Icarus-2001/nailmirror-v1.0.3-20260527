...
云未就绪时会自动降级 Mock 登录（仍保留弹层头像昵称），不影响「先随便逛逛」。小程序版本见 `app.globalData.version`（当前 **1.2.2**）。登录页已内置 `privacy-popup`，首屏须先同意隐私再选头像昵称。
...
修改后重新编译小程序。变更摘要见 [`CHANGELOG.md`](./CHANGELOG.md)（当前小程序版本 **1.1.1**）与 [`TRYON_0531稳定出图策略.md`](./TRYON_0531稳定出图策略.md)。

### 8.2 试戴入口与步骤（v1.1.1）

| 入口 | 步骤数 | 流程 |
|------|--------|------|
| 首页大卡 **立即试戴** | **四步** | 选甲型 → 选款式 → 上传照片 → 生成预览 |
| 款式详情 **立即试戴** | **三步** | 选甲型 → 上传照片 → 生成预览（款式已定，URL 带 `styleId`） |

实现要点：`try-on-static` **仅根据 URL 是否带 `styleId`** 切换三步/四步，**不读** `tryOnStore.currentStyleId`（避免先打开商详后回首页误走三步）。首页进入试戴前会清空 store 中的款式 ID。

### 8.3 试戴操作路径

1. 编译 → 首页 → **立即试戴**（四步）或 款式库/推荐 → 商详 → **立即试戴**（三步）
2. 手照步骤（三选一或组合）：
   - 点 **「拍照」** 或 **「从相册选择」**（需隐私指引已发布，见 8.5）
