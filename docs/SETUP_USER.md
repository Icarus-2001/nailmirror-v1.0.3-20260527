...
| `WANX_EDIT_MODEL` | 可选 | 默认 `wanx2.1-imageedit`（勿写成 `wan2.1-imageedit`） |
...
期望返回含：`hasDashScopeKey: true`、`wanModel: "wanx2.1-imageedit"`、`runtime: "handler-v5-eval-stable"`
...
3. 等待约 30–90 秒，结果页应出现 AI 合成图
4. 控制台可看 `[tryon-cloud]` 日志
...
| `Model not exist` | 确认 `WANX_EDIT_MODEL=wanx2.1-imageedit` |
