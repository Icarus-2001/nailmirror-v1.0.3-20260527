...
3. **登录页（1.1.4+）**：首屏会先弹出隐私同意（须已配置用户头像/昵称等声明）；同意后点「微信一键登录」→ 弹层选头像、选用微信昵称 → 确认登录；资料存本地并在「我的」展示（云函数不落库）
...
**入口（v1.1.1）**：首页 **立即试戴** = 四步（选甲型 → 选款式 → 上传照片 → 预览）；款式详情 **立即试戴** = 三步（跳过选款）。见 [`CHANGELOG.md`](./CHANGELOG.md) **1.1.1**。

1. 编译 → 首页 **立即试戴**（四步）或 商详 **立即试戴**（三步）
2. 选手照方式（三选一）：
