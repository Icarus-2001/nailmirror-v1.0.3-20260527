...
## 1.2.11 · 2026-06-07 · 热度榜单规则说明入口

**小程序版本：`1.2.11`**（`app.globalData.version`）

### 一句话（版本说明可用）

**热度榜单「站外榜单 / 站内榜单」标题旁增加「榜单规则」链接，点击可查看对应热度排序说明，帮助用户理解榜单数字含义。**

### 榜单规则说明弹窗

- **`pages/hot-rank`**：标题行右侧增加「榜单规则 ›」文字链；点击弹出可滚动说明弹窗，底部「知道了」或点遮罩关闭。
- **站外榜单规则**：说明全网热款来源（小红书等站外互动热度）、排序口径、批次更新方式，以及与站内热度的区别。
- **站内榜单规则**：说明仅含【平台特供】【来自商家】、每日 10:00 更新、近 30 天浏览/收藏/试戴综合排序及新鲜度回落逻辑（用户向文案，无公式术语）。
- **`config/hot-rank-rules.js`**：集中维护两套规则文案；切换 Tab 后再点链接展示对应规则。

### 涉及文件

- `config/hot-rank-rules.js`（新）
- `pages/hot-rank/index.{js,wxml,wxss}`
- `tests/config/hot-rank-rules.test.js`（新）
- `app.js`、`package.json`
...
- 「热度榜单」→「站外榜单」→ 点「榜单规则 ›」→ 显示站外规则，可滚动关闭。
- 切「站内榜单」→ 再点链接 → 显示站内规则（含 10:00 更新、30 天、浏览/收藏/试戴）。
- 不点链接时榜单展示与改前一致。
- `npm test -- --testPathPattern=hot-rank-rules.test` 通过。
...
**「我的」页点击头像弹出资料弹窗，支持微信头像、相册选图与昵称确认；未登录可在此完成登录。**

### 我的页头像与资料编辑

- **问题**：静默 `openid` 或已登录用户无法在「我的」页修改头像；未登录时点头像无登录入口。
- **`pages/me`**：头像增加「换头像」角标，`catchtap` 打开资料弹窗（`chooseAvatar` + 昵称，与登录页一致）。
- **选图方式**：圆形按钮选微信头像；「从相册选择图片」走 `chooseMedia`（选图前 `ensurePrivacyAuthorized`）。
- **未登录**：弹窗标题「微信登录」，确认后 `userService.login`。
- **已登录**：弹窗标题「更换头像」，确认后 `userStore.setUser` 本地更新昵称/头像。
- 未登录时点击头部信息区仍可打开同一弹窗。
...
- 「我的」→ 点击头像 → 弹出资料弹窗。
- 选微信头像或相册图片 + 昵称 → 保存后头像与昵称更新。
- 未登录用户通过弹窗可完成登录。

---

## 1.2.9 · 2026-06-07 · 商家分包 hot-rank 回归修复

**小程序版本：`1.2.9`**（`app.globalData.version`）

### 一句话（版本说明可用）

**修复 1.2.8 将 `pages-b/hot-rank` 重新注册到商家分包导致预加载失败、无法进入上传款式的问题。**

### 商家分包 hot-rank 回归修复

- **问题**：1.2.8 为兼容旧缓存将 `pages-b/hot-rank/index` 加回 `app.json` 商家分包；首页 `preloadRule` 预加载 merchant 分包时该桩页报 `module 'pages-b/hot-rank/index.js' is not defined`，整个 B 端分包不可用，「上传款式」等页面无法打开。
- **`app.json`**：从商家分包**移除** `hot-rank/index`（与 1.2.4 下线策略及 `stock-advice` 一致）；磁盘保留 `pages-b/hot-rank/*` 兼容桩，**勿重新注册**。
- **`pages-b/hot-rank`**：桩页去掉 `loading` 组件依赖，改为极简跳转结构。

### 涉及文件

- `app.json`
- `pages-b/hot-rank/index.{js,json,wxml}`
- `app.js`、`package.json`
...
- 控制台无 `pages-b/hot-rank/index.js is not defined` 报错。
- 「我的」→ 商家经营入口 → **上传款式** → 页面正常打开、可选图。
- 1.2.8 已有能力（款式库来源筛选、B 端隐私选图）仍正常。
